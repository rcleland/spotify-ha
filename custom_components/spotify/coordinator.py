...
def _peek_next_from_queue_json(raw: str) -> tuple[str | None, str | None, str | None]:
    """Parse first item from GET /v1/me/player/queue for Lovelace 'up next' widgets."""
    try:
    ...
    thumb: str | None = None
    artist_line: str | None = None
    if media_type == "episode":
        show = item.get("show") or {}
        artist_line = show.get("name")
        imgs = item.get("images") or show.get("images") or []
        if imgs:
...
class SpotifyCoordinator(DataUpdateCoordinator[SpotifyCoordinatorData]):
    ...
    async def _async_fetch_queue_peek(self) -> tuple[str | None, str | None, str | None]:
        """First entry in the user's playback queue (Spotify's 'up next'), if any."""
        try:
            raw = await self.client._get("v1/me/player/queue")  # noqa: SLF001
        except SpotifyNotFoundError:
            return None, None, None
        except SpotifyConnectionError:
            return None, None, None
        except Exception:  # noqa: BLE001
            _LOGGER.debug("Could not read Spotify playback queue", exc_info=True)
            return None, None, None
        return _peek_next_from_queue_json(raw)
    ...
    async def _async_update_data(self) -> SpotifyCoordinatorData:
        ...
        queue_next = await self._async_fetch_queue_peek()
        ...
        if not current:
            return SpotifyCoordinatorData(
                ...
                playlist=None,
                dj_playlist=False,
                next_title=queue_next[0],
                next_artist=queue_next[1],
                next_thumbnail=queue_next[2],
            )
